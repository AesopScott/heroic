...
namespace Heroic.Enemies
{
    public class EnemySpawner : MonoBehaviour
    {
        ...
        private EnemyDefinition ChooseCrashDefinition(WaveDefinition activeWave)
        {
            ...
            if (level >= 6)
            {
                EnemyDefinition wallDefinition = FindEnemyDefinition(activeWave, "enemy_wall_01");
                if (wallDefinition != null && Random.value < 0.2f)
                {
                    return wallDefinition;
                }
            }

            int crashLevel = Mathf.Clamp(level, 1, 5);
